...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			local commit = readfile('tumbavape/profiles/commit.txt'):gsub('%s', '')
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('tumbavape/assets/new/blur.png')
...
local function serverHop(pointer, filter)
	...
	if not pointer then
		notif('Tumba Vape', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Tumba Vape', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Tumba Vape', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Tumba Vape', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
...
tumbahub.Libraries.string = loadstring(downloadFile('tumbavape/libraries/string.lua'), 'string')()
...
local hash = loadstring(downloadFile('tumbavape/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('tumbavape/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('tumbavape/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
run(function()
		...
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				tumbahub.Uninject = function()
					notif('Tumba Vape', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('Tumba Vape', plr.Name..' is using tumbahub!', 60)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('tumbavape/profiles/whitelist.json') and readfile('tumbavape/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('tumbavape/profiles/whitelist.json', whitelist.textdata)
				end)
			end
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('tumbavape/assets/new/arrowmodule.png')
...
run(function()
	...
	Radar = tumbahub:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('tumbavape/assets/new/radaricon.png'),
...
run(function()
	...
	local function ChangeFFlag(suc)
		...
		if not success or typeof(json) ~= 'table' then
			notif('Tumba Vape', 'Invalid json format for fflag', 12, 'warning')
			return
		end
		...
		notif('Tumba Vape', 'FFlags applied, Go in a new game to take effect', 12, 'info')
	end
	...
	FFlag = tumbahub.Legit:CreateModule({
		...
		Function = function(call)
			if call then
				...
				notif('Tumba Vape', 'Inorder to disable fflags you have applied, You need to restart roblox', 20, 'info')
			end
		end
	})
