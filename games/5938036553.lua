...
local drawingactor = loadstring(downloadFile('tumbavape/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		...
		task.spawn(function()
			...
			local executionString = "loadfile('tumbavape/main.lua')("..drawingactor..")"
			...
			notif('Tumba Vape', 'Failed to find actor', 10, 'alert')
		end)
	else
		tumbahub.Load = function()
			notif('Tumba Vape', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('Tumba Vape', 'Failed to hook ('..id..')', 10, 'alert')
	end
