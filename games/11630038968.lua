...
run(function()
	...
	task.spawn(function()
		...
		if map and tumbahub.Loaded ~= nil then
			tumbahub:Clean(map.DescendantAdded:Connect(function(v)
				parsePositions(v, function(pos)
					store.blocks[pos] = v
				end)
			end))
			tumbahub:Clean(map.DescendantRemoving:Connect(function(v)
				parsePositions(v, function(pos)
...
run(function()
	...
	tumbahub.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPlay = tumbahub.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
